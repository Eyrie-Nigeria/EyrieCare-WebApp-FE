...
export function ProfileView({ userType = "student" }: ProfileViewProps) {
  ...
  const footerText = {
    student: "Student Profile Console",
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
            ...
            >
              <tab.icon
                className={cn(
                  ...
                  isActive ? "text-primary-dashboard" : "opacity-70",
                )}
              />
              {tab.label}
            </button>
      ...
      <footer className="mt-8 py-6 text-center text-text-dashboard-secondary-light dark:text-slate-600 text-xs font-medium border-t border-slate-100 dark:border-card-dashboard-dark/30">
        <p>
          © {new Date().getFullYear()} EyrieCare Medical Platform.{" "}
          {footerText[userType]}.
        </p>
      </footer>
    </div>
